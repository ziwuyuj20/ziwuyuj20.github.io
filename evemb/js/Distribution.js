...
function setCityBack(pindex){
	...
	$(".Address_plug_province_box_2").innerHTML = "";//清空操作
	var str = '';
	for (var i=0;i<city.length;i++) {
		str += '<li value="'+i+'" class="ico_2">'+city[i]["-name"]+'</li>'
	}
	$(".Address_plug_province_box_2").html(str);
}
//设置区县下拉框
function setCountyBack(pindex,cindex){
	var county = province[pindex].city[cindex].county;
	$(".Address_plug_province_box_3").innerHTML = "";//清空操作
	var str = '';
	for (var i=0;i<county.length;i++) {
		str += '<li value="'+i+'" class="ico_3">'+county[i]["-name"]+'</li>'
	}
	$(".Address_plug_province_box_3").html(str);
}
//生成省份
$(".Address_plug").on("click",function(){
	$(window).scrollTop("0");
	$(".popupeHead,.center_out,.action_bar,.top,.footer_h").hide();
...
$(document).on("click",".ico_3",function(){
	...
	$(".popupeHead,.center_out,.action_bar,.top,.footer_h").show();
	area_txt = $(this).text();
